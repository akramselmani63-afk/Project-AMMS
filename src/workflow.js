...
export function migrate(db) {
  if (db.schemaVersion >= 3) return db;
  ...
  for (const w of db.workOrders) {
    ...
    if (!w.requestId) {
      const r={id:nextId(db.requests,'IR'),title:w.title,equipmentId:w.equipmentId,description:w.notes || '',status:'Submitted',severity:'S2',priority:w.priority || 'P3',reportedBy:w.assignee || 'Legacy demo',createdAt:today(),photos:[],risks:[],history:[],legacyWorkId:w.id};
...
export function addRequest(db,v) {
  ...
  const r={id:nextId(db.requests,'IR'),title:required(v,'title'),equipmentId:v.equipmentId,description:required(v,'description'),impact:v.impact || '',reportedBy:required(v,'reportedBy'),createdAt:today(),status:'Submitted',severity:null,priority:null,photos:v.photos || [],risks:[],history:[]};
...
export function assess(db,id,v) {
  ...
  requireValue(['S1','S2','S3','S4'].includes(v.severity) && ['P1','P2','P3','P4'].includes(v.priority),'Select severity and priority.');
  Object.assign(r,{severity:v.severity,priority:v.priority,diagnosis:required(v,'diagnosis'),risks:v.risks || [],status:'Responsible review'});
  delete r.approval; delete r.hseApproval; audit(db,r,'Assessed',r.diagnosis);
}
export function reviewRequest(db,id,decision,v) {
  const r=record(db,'requests',id);
  if(r.status==='Responsible review') allow(db,'approve'); else { stage(r,'HSE review'); allow(db,'hse'); }
  requireValue(['approve','return','reject'].includes(decision),'Invalid decision.');
  const note=required(v,'note');
  if(decision!=='approve') { r.status=decision==='return'?'Returned':'Rejected'; audit(db,r,r.status,note); return; }
  if(r.status==='Responsible review') { r.approval=audit(db,r,'Responsible approval',note); r.status='HSE review'; }
  else {
    r.precautions=note; r.hseApproval=audit(db,r,'HSE approval',note); r.status='Approved';
    for(const w of db.workOrders.filter(w=>w.requestId===id && w.status==='Awaiting approval')) w.status='Planned';
  }
}
export function createWork(db,id,v) {
  allow(db,'work'); const r=record(db,'requests',id); stage(r,'Approved');
  ...
  const w={id:nextId(db.workOrders,'WO'),title:r.title,equipmentId:r.equipmentId,requestId:id,pmId:r.pmId || null,type:r.pmId?'Preventive':'Corrective',priority:r.priority,status:'Planned',dueDate:required(v,'dueDate'),participants,external:v.external || '',notes:v.notes || '',history:[]};
...
export function updateWork(db,id,action,v={}) {
  ...
  if(action==='start') {
    ...
    requireValue(!partsPending(db,w),'Required spare parts are still awaiting acceptance.');
    const completion={diagnosis:required(v,'diagnosis'),cause:required(v,'cause'),actions:required(v,'actions'),condition:required(v,'condition'),confirmation:required(v,'confirmation'),downtimeMinutes:number(v.downtimeMinutes),repair:required(v,'repair')};
...
export function closeWork(db,id,v) {
  ...
  if(w.status==='HSE closure') { allow(db,'hse'); w.safetyClosure=audit(db,w,'HSE closure',required(v,'note')); w.status='Validation'; return; }
  stage(w,'Validation'); allow(db,'close');
  const note=required(v,'note');
  if(w.participants.includes(db.role)) {
    requireValue(w.participants.length===2 && v.exception?.trim(),'An independent maintenance reviewer is required, or a documented exception for joint work.');
  }
  w.validation=audit(db,w,'Closed',note); w.validation.exception=v.exception || ''; w.status='Closed';
...
export function updatePart(db,id,action,v) {
  ...
  if(action==='approve' || action==='reject') {
    allow(db,'approve'); stage(p,'Responsible review'); const note=required(v,'note');
    p.status=action==='approve'?'Purchasing':'Rejected'; audit(db,p,p.status,note);
  } else if(action==='order') {
    allow(db,'purchase'); stage(p,'Purchasing');
    Object.assign(p,{supplier:required(v,'supplier'),orderReference:required(v,'orderReference'),expectedDate:required(v,'expectedDate'),quote:required(v,'quote'),status:'Ordered'}); audit(db,p,'Ordered',p.orderReference);
    ...
    const note=required(v,'note'); Object.assign(d,{accepted,rejected:d.quantity-accepted,note,checkedAt:now()});
    ...
    allow(db,'accept'); stage(p,'Accepted'); p.status='Closed'; audit(db,p,'Closed',required(v,'note'));
  }
}
...
export function addPM(db,v) {
  ...
  const p={id:nextId(db.preventive,'PM'),title:required(v,'title'),equipmentId:v.equipmentId,intervalDays,nextDue:required(v,'nextDue'),owner:db.role,instructions:required(v,'instructions')};
...
export function addReport(db,v) {
  ...
  const r={id:nextId(db.reports,'SR'),date,shift:v.shift || 'Day',author:db.actor || db.role,summary:required(v,'summary'),handover:v.handover || '',activities,status:'Draft',history:[]};
  db.reports.unshift(r); audit(db,r,'Report created'); return r;
}
export function approveReport(db,id,v) { allow(db,'approve'); const r=record(db,'reports',id); stage(r,'Draft'); r.approval=audit(db,r,'Report approved',required(v,'note')); r.status='Approved'; }
