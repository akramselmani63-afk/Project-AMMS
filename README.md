...
- Reports have separate Intervention reports and Shift reports views. Each intervention card combines one request with its work order and is grouped by latest activity date; a date filter narrows the list. Written shift reports add observations, diagnosis, risks, root cause, result and handover notes, with compact links to that date's interventions. The author specifies shift allocation in notes; a written report is approved by the Responsible.
