...
- Reports have separate Intervention reports and Shift reports views. Each intervention card combines one request with its work order and is grouped by latest activity date; a date filter narrows the list. Written shift reports need only date, shift, optional observations and handover notes, with compact links to that date's interventions. Technical fields remain visible on older reports that already contain them. A written report is approved by the Responsible.
